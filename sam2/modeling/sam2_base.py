            ...
            if self.samurai_mode:
                ...
                if frame_idx > 1:  # Ensure we have previous frames to evaluate
                    for i in range(frame_idx - 1, 1, -1):  # Iterate backwards through previous frames
                        # 检查帧是否存在于non_cond_frame_outputs中
                        if i not in output_dict["non_cond_frame_outputs"]:
                            continue  # 如果帧不存在，跳过此帧
                            
                        iou_score = output_dict["non_cond_frame_outputs"][i].get("best_iou_score", None)
                        if iou_score is None:
                            continue  # 如果没有iou分数，跳过此帧
                            
                        obj_score = output_dict["non_cond_frame_outputs"][i].get("object_score_logits", None)
                        if obj_score is None:
                            continue  # 如果没有object_score，跳过此帧
